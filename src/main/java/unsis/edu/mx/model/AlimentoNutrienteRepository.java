...
public interface AlimentoNutrienteRepository extends JpaRepository<AlimentoNutriente, Long> {
	...
	/*@Query(value = "SELECT alinut.id_alimento_nutriente, alinut.alimento_id, alinut.nutriente_id, nut.nutriente, alinut.cantidad FROM alimento AS ali INNER JOIN alimento_nutriente AS alinut ON ali.id_alimento = alinut.alimento_id INNER JOIN nutrientes AS nut ON alinut.nutriente_id = nut.id_nutrientes WHERE ali.id_alimento=:idAlimento", nativeQuery=true)
